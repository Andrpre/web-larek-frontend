...
В проекте используется паттерн MVP для организации архитектуры. 
...
В роли представления выступают компоненты отображения, такие как `Card`, `Form`, `Basket` и другие, которые расширяют абстрактный класс `Component<T>`. Они отображают данные и реагируют на действия пользователя. Представления получают данные от модели и отображают их на экране, а также передают пользовательские действия (например, клики или ввод данных) презентеру для обработки.

**Презентер (Presenter):**

Для данного приложения презентером ****является код, который находится в корневом `index.ts`, там мы соединяем через колбэки или события компоненты и API.

**Компоненты коммуникации:**

Слой коммуникации является частью слоя презентер — это API-клиент. Классы `ShopApi` и `Api` используются в презентере для выполнения запросов к серверу и получения данных.

Связь между слоями осуществляется путем использования объектов и методов из одного слоя в другом. Компоненты отображения могут использовать модели данных для получения и обновления данных, а также обращаться к API для выполнения запросов к серверу, через слой презентер . События могут использоваться для уведомления компонентов о изменениях состояния или выполнении определенных действий. Класс `EventEmitter` предоставляет механизм для этого, позволяя классам подписываться на события и реагировать на них без явной зависимости друг от друга.
...
- `basket: string[]` — массив строк, представляющий элементы в корзине.
- `catalog: ProductItem[]` — массив объектов `ProductItem`, представляющих элементы каталога.
- `loading: boolean` —  флаг, указывающий на состояние загрузки.
- `order: IOrder` —  объект, представляющий заказ, с полями `payment`, `address`, `email`, `phone` и `items`.
- `preview: string | null` —  идентификатор элемента для предпросмотра или `null`, если предпросмотр не активен.
...
- `setPreview(item: IProductItem)` — открывает модальное окно для более детальной информации.
- `setOrderField(field: keyof IOrderForm, value: string)` — устанавливает значение поля заказа и, если заказ прошел валидацию, сообщает о готовности заказа.
...
`_title`, `_image`, `_category`, `_price`, `_description`, `_button`: приватные свойства, представляющие заголовок, изображение, категорию, стоимость и кнопку.

`_description`, `_button`: являются не обязательными свойствами и заполняются в зависимости от контекста (например не актуальны для отображения в каталоге, но актуальны в модальном окне).
...
- `image` — сеттер для изображения карточки (позволяет устанавливать изображение).
- `description` — сеттер для описания карточки. Может принимать как строку, так и массив строк для многострочного описания (позволяет устанавливать описание товара).
...
2. `events` — объект событий, который используется для управления событиями формы. Он предоставляет метод `emit` для генерации событий и другие части кода могут подписаться на это событие и реагировать соответствующим образом.
...
- `totalCost: number` — позволяет отображать итоговую сумму покупки.
